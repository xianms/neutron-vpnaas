...
from neutron.extensions import l3_ext_ha_mode as vpn_ha
...
VPN_HA_OPTS = [
    ...
    cfg.IntOpt('max_vpn_agents_per_router',
               default=3,
               help=_("Maximum number of VPN agents which a HA router will be "
                      "scheduled on. If it is set to 0 then the router will "
                      "be scheduled on every agent.")),
    cfg.IntOpt('min_vpn_agents_per_router',
               default=n_const.DEFAULT_MINIMUM_AGENTS_FOR_HA,
               help=_("DEPRECATED: Minimum number of VPN agents that have to "
    ...
    cfg.StrOpt('vpn_ha_net_cidr',
               default=n_const.L3_HA_NET_CIDR,
               help=_('Subnet used for the vpn HA admin network.')),
    cfg.StrOpt('vpn_ha_network_type', default='',
    ...
    cfg.StrOpt('vpn_ha_network_physical_name', default='',
               help=_("The physical network name with which the HA network "
                      "can be created."))
]
cfg.CONF.register_opts(VPN_HA_OPTS)


class VPNHARouterAgentPortBinding(model_base.BASEV2):
    ...
    __tablename__ = 'ha_vpn_router_agent_port_bindings'
    __table_args__ = (
        sa.UniqueConstraint(
            'router_id', 'vpn_agent_id',
            name='uniq_ha_vpn_router_agent_port_bindings0port_id0vpn_agent_id'),
...
class VPNHARouterNetwork(model_base.BASEV2, model_base.HasProjectPrimaryKey):
    ...
    __tablename__ = 'ha_vpn_router_networks'
...
class VPNHARouterVRIdAllocation(model_base.BASEV2):
    """VRID allocation per HA network.

    Keep a track of the VRID allocations per HA network.
    """

    __tablename__ = 'ha_vpn_vpn_router_vrid_allocations'
...
class VPN_HA_NAT_db_mixin(router_az_db.RouterAvailabilityZoneMixin):
    ...
    '''
    extra_attributes = (
        vpn_dvr_db.VPN_NAT_with_dvr_db_mixin.extra_attributes +
        router_az_db.RouterAvailabilityZoneMixin.extra_attributes + [
            {'name': 'ha', 'default': cfg.CONF.vpn_ha},
            {'name': 'ha_vr_id', 'default': 0}])
    '''

    def _verify_configuration(self):
        self.ha_cidr = cfg.CONF.vpn_ha_net_cidr
        try:
            net = netaddr.IPNetwork(self.ha_cidr)
        except netaddr.AddrFormatError:
            raise vpn_ha.HANetworkCIDRNotValid(cidr=self.ha_cidr)
    ...
    def _check_num_agents_per_router(self):
        max_agents = cfg.CONF.max_vpn_agents_per_router
        min_agents = cfg.CONF.min_vpn_agents_per_router

        if (max_agents != UNLIMITED_AGENTS_PER_ROUTER
            and max_agents < min_agents):
            raise vpn_ha.HAMaximumAgentsNumberNotValid(
                max_agents=max_agents, min_agents=min_agents)

        if min_agents < n_const.MINIMUM_MINIMUM_AGENTS_FOR_HA:
            raise vpn_ha.HAMinimumAgentsNumberNotValid()

    def __init__(self):
        self._verify_configuration()
        super(VPN_HA_NAT_db_mixin, self).__init__()

    def get_ha_network(self, context, tenant_id):
        return (context.session.query(VPNHARouterNetwork).
                filter(VPNHARouterNetwork.tenant_id == tenant_id).
                first())

    def _get_allocated_vr_id(self, context, network_id):
        with context.session.begin(subtransactions=True):
            query = (context.session.query(VPNHARouterVRIdAllocation).
                     filter(VPNHARouterVRIdAllocation.network_id == network_id))
    ...
    def _allocate_vr_id(self, context, network_id, router_id):
            ...
            try:
                ...
                with context.session.begin():
                    ...
                    if not available_vr_ids:
                        raise vpn_ha.NoVRIDAvailable(router_id=router_id)

                    allocation = VPNHARouterVRIdAllocation()
        ...
        raise vpn_ha.MaxVRIDAllocationTriesReached(
    ...
    def _delete_vr_id_allocation(self, context, ha_network, vr_id):
        with context.session.begin(subtransactions=True):
            context.session.query(VPNHARouterVRIdAllocation).filter_by(
    ...
    def _create_ha_network_tenant_binding(self, context, tenant_id,
                                          network_id):
        with context.session.begin():
            ha_network = VPNHARouterNetwork(tenant_id=tenant_id,
                                           network_id=network_id)
            context.session.add(ha_network)
        # we need to check if someone else just inserted at exactly the
        # same time as us because there is no constrain in VPNHARouterNetwork
        # that prevents multiple networks per tenant
        with context.session.begin(subtransactions=True):
            items = (context.session.query(VPNHARouterNetwork).
    ...
    def _add_ha_network_settings(self, network):
        if cfg.CONF.vpn_ha_network_type:
            network[providernet.NETWORK_TYPE] = cfg.CONF.vpn_ha_network_type

        if cfg.CONF.vpn_ha_network_physical_name:
            network[providernet.PHYSICAL_NETWORK] = (
                cfg.CONF.vpn_ha_network_physical_name)
    ...
    def get_number_of_agents_for_scheduling(self, context):
        ...
        min_agents = cfg.CONF.min_vpn_agents_per_router
        num_agents = len(self.get_vpn_agents(context, active=True,
            filters={'agent_modes': [constants.VPN_AGENT_MODE_LEGACY,
                                     constants.VPN_AGENT_MODE_DVR_SNAT]}))
        max_agents = cfg.CONF.max_vpn_agents_per_router
        if max_agents:
            if max_agents > num_agents:
                LOG.info(_LI("Number of active agents lower than "
                             "max_vpn_agents_per_router. VPN agents "
                             "available: %s"), num_agents)
            else:
                num_agents = max_agents

        if num_agents < min_agents:
            raise vpn_ha.HANotEnoughAvailableAgents(min_agents=min_agents,
        ...
        try:
            ...
            with excutils.save_and_reraise_exception() as ctxt:
                if isinstance(e.inner_exception, sql_exc.IntegrityError):
                    ...
                    raise vpn.RouterNotFound(router_id=router_id)
    ...
    def _notify_ha_interfaces_updated(self, context, router_id,
        ...
        self.vpn_rpc_notifier.routers_updated(
    ...
    def _get_device_owner(self, context, router=None):
        ...
        return super(VPN_HA_NAT_db_mixin,
    ...
    def create_vpn_router(self, context, router):
        ...
        if is_ha:
            try:
                ...
                self.schedule_router(context, router['id'])
                '''
    ...
    def _update_router_db(self, context, router_id, data):
        ...
        with context.session.begin(subtransactions=True):
            router_db = super(VPN_HA_NAT_db_mixin, self)._update_router_db(
    ...
    def delete_vpn_router(self, context, id):
        router_db = self._get_router(context, id)
        super(VPN_HA_NAT_db_mixin, self).delete_router(context, id)
    ...
    def _unbind_ha_router(self, context, router_id):
        for agent in self.get_vpn_agents_hosting_routers(context, [router_id]):
            self.remove_router_from_vpn_agent(context, agent['id'], router_id)

    def get_ha_router_port_bindings(self, context, router_ids, host=None):
        if not router_ids:
            return []
        query = context.session.query(VPNHARouterAgentPortBinding)
        ...
        query = query.filter(
            VPNHARouterAgentPortBinding.router_id.in_(router_ids))
    ...
    def _check_router_agent_ha_binding(context, router_id, agent_id):
        query = context.session.query(VPNHARouterAgentPortBinding)
        query = query.filter(
            VPNHARouterAgentPortBinding.router_id == router_id,
            VPNHARouterAgentPortBinding.vpn_agent_id == agent_id)
    ...
    def get_vpn_bindings_hosting_router_with_ha_states(
    ...
    def _process_sync_ha_data(self, context, routers, host, agent_mode):
        ...
        return [r for r in list(routers_dict.values())
                if (agent_mode == constants.VPN_AGENT_MODE_DVR or
    ...
    def get_ha_sync_data_for_host(self, context, host, agent,
        ...
        dvr_agent_mode = (agent_mode in [constants.VPN_AGENT_MODE_DVR_SNAT,
                                         constants.VPN_AGENT_MODE_DVR])
        if (dvr_agent_mode and n_utils.is_extension_supported(
                self, constants.VPN_DISTRIBUTED_EXT_ALIAS)):
            # DVR has to be handled differently
            sync_data = self._get_dvr_sync_data(context, host, agent,
                                                router_ids, active)
        else:
            sync_data = super(VPN_HA_NAT_db_mixin, self).get_sync_data(context,
                                                            router_ids, active)
