...
class VPNScheduler(object):
    ...
    @property
    def l3_plugin(self):
        return manager.NeutronManager.get_service_plugins().get(L3_ROUTER_NAT)
    ...
    def _router_has_binding(self, context, router_id, vpn_agent_id):
        router_binding_model = vpn_agentschedulers_db.RouterVPNAgentBinding

        query = context.session.query(router_binding_model)
        query = query.filter(router_binding_model.router_id == router_id,
                             router_binding_model.vpn_agent_id == vpn_agent_id)
    ...
    def _filter_unscheduled_routers(self, context, plugin, routers):
        ...
        for router in routers:
            vpn_agents = plugin.get_vpn_agents_hosting_routers(
                context, [router['id']])
            if vpn_agents:
                LOG.debug('Router %(router_id)s has already been '
                          'hosted by VPN agent %(agent_id)s',
                          {'router_id': router['id'],
                           'agent_id': vpn_agents[0]['id']})
            else:
    ...
    def _get_unscheduled_routers(self, context, plugin):
        ...
        if unscheduled_router_ids:
            return self.l3_plugin.get_router(
    ...
    def _get_routers_to_schedule(self, context, plugin, router_ids=None):
        ...
        if router_ids is not None:
            ...
            routers = self.l3_plugin.get_router(context, filters=filters)
    ...
    def _get_routers_can_schedule(self, context, plugin, routers, vpn_agent):
        """Get the subset of routers that can be scheduled on the VPN agent."""
        ids_to_discard = set()
        for router in routers:
            # check if the vpn agent is compatible with the router
            candidates = plugin.get_vpn_agent_candidates(
                context, router, [vpn_agent])
            if not candidates:
    ...
    def auto_schedule_routers(self, plugin, context, host, router_ids):
        """Schedule non-hosted routers to VPN Agent running on host.
    ...
    def _get_candidates(self, plugin, context, sync_router):
        ...
        with context.session.begin(subtransactions=True):
            ...
            # one enabled vpn agent hosting since active is just a
            # timing problem. Non-active vpn agent can return to
            # active any time
            ...
            if current_vpn_agents:
                LOG.debug('Router %(router_id)s has already been hosted '
                          'by VPN agent %(agent_id)s',
                          {'router_id': sync_router['id'],
                           'agent_id': current_vpn_agents[0]['id']})
                return []

            active_vpn_agents = plugin.get_vpn_agents(context, active=True)
            if not active_vpn_agents:
                LOG.warning(_LW('No active VPN agents'))
                return []
            candidates = plugin.get_vpn_agent_candidates(context,
                                                        sync_router,
                                                        active_vpn_agents)
            if not candidates:
                LOG.warning(_LW('No VPN agents can host the router %s'),
                            sync_router['id'])
            ...
            if router.get('ha'):
                if not self._router_has_binding(context, router['id'],
                    ...
                    self.create_ha_port_and_bind(
                        ...
                        router['tenant_id'], vpn_agent)
            else:
                self.bind_router(context, router['id'], vpn_agent)

    def bind_router(self, context, router_id, chosen_agent):
        """Bind the router to the vpn agent which has been chosen."""
        try:
            with context.session.begin(subtransactions=True):
                binding = vpn_agentschedulers_db.RouterVPNAgentBinding()
                binding.vpn_agent = chosen_agent
                binding.router_id = router_id
                context.session.add(binding)
        except db_exc.DBDuplicateEntry:
            LOG.debug('Router %(router_id)s has already been scheduled '
                      'to VPN agent %(agent_id)s.',
        ...
        LOG.debug('Router %(router_id)s is scheduled to VPN agent '
    ...
    def create_ha_port_and_bind(self, plugin, context, router_id,
        ...
        try:
            ...
            with db_api.autonested_transaction(context.session):
                port_binding.vpn_agent_id = agent['id']
        except db_exc.DBDuplicateEntry:
    ...
    def get_ha_routers_vpn_agents_counts(self, context, plugin, filters=None):
        """Return a mapping (router, # agents) matching specified filters."""
        return plugin.get_ha_routers_vpn_agents_count(context)
    ...
    def _schedule_ha_routers_to_additional_agent(self, plugin, context, agent):
        ...
        to be scheduled on the given agent if max_vpn_agents_per_router
        is not yet reached.
        """

        routers_agents = self.get_ha_routers_vpn_agents_counts(context, plugin,
                                                              agent)
        scheduled = False
    ...
    def _bind_ha_router_to_agents(self, plugin, context, router_id,
        ...
        for port_binding, agent in zip(port_bindings, chosen_agents):
            try:
                ...
                LOG.debug('HA Router %(router_id)s is scheduled to VPN agent '
                          '%(agent_id)s)',
...
class LeastRoutersScheduler(VPNScheduler):
    ...
    def _choose_router_agent(self, plugin, context, candidates):
        ...
        chosen_agent = plugin.get_vpn_agent_with_min_routers(
            context, candidate_ids)
        return chosen_agent

    def _choose_router_agents_for_ha(self, plugin, context, candidates):
        num_agents = self._get_num_of_agents_for_ha(len(candidates))
        ordered_agents = plugin.get_vpn_agents_ordered_by_num_routers(
...
class AZLeastRoutersScheduler(LeastRoutersScheduler):
    """Availability zone aware scheduler.
       ...
       If a router is ha router, allocate VPN agents distributed AZs
    ...
    def get_ha_routers_vpn_agents_counts(self, context, plugin, filters=None):
        ...
        all_routers_agents = (
            ...
            get_ha_routers_vpn_agents_counts(context, plugin, filters))
        if filters is None:
